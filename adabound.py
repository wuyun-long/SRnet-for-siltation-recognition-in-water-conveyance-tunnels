...
class AdaBound(Optimizer):
    ...
    # Arguments
        ...
        epsilon: float >= 0. Fuzz factor. If `None`, defaults to `backend.epsilon()`.
    ...
    def get_updates(self, loss, params):
        ...
        if self.initial_decay > 0:
            lr = lr * (1. / (1. + self.decay * backend.cast(self.iterations,
                                                      backend.dtype(self.decay))))

        t = backend.cast(self.iterations, backend.floatx()) + 1

        # Applies bounds on actual learning rate
        step_size = lr * (backend.sqrt(1. - backend.pow(self.beta_2, t)) /
        ...
        for p, g, m, v, vhat in zip(params, grads, ms, vs, vhats):
            ...
            if self.weight_decay != 0.:
                g += self.weight_decay * backend.stop_gradient(p)

            m_t = (self.beta_1 * m) + (1. - self.beta_1) * g
            v_t = (self.beta_2 * v) + (1. - self.beta_2) * backend.square(g)

            if self.amsbound:
                vhat_t = backend.maximum(vhat, v_t)
                denom = (backend.sqrt(vhat_t) + self.epsilon)
                self.updates.append(backend.update(vhat, vhat_t))
            else:
                denom = (backend.sqrt(v_t) + self.epsilon)

            # Compute the bounds
            step_size_p = step_size * backend.ones_like(denom)
            step_size_p_bound = step_size_p / denom
            bounded_lr_t = m_t * backend.minimum(backend.maximum(step_size_p_bound,
